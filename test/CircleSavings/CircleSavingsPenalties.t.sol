...
contract CircleSavingsPenalties is CircleSavingsSetup {
    ...
    function test_LateContributionDeductsCollateralAndMarksLate() public {
        ...
        deal(address(USDT), alice, contribution);
        // Bob only needs the contribution amount now (late fee comes from collateral)
        deal(address(USDT), bob, contribution);
        deal(address(USDT), charlie, contribution);
        deal(address(USDT), david, contribution);
        deal(address(USDT), eve, contribution);
    ...
    function test_RoundCompletePayoutAndReputationIncrease() public {
        ...
        assertEq(aliceBalAfter - aliceBalBefore, 500e6 - 100e6);
    ...
    function test_Payout_NonCreatorWithPlatformFee() public {
        ...
        assertGt(balAfter, balBefore);
        // Payout should be less than full 500e6 due to 1% fee
        assertLt(balAfter - balBefore, 500e6);
    }

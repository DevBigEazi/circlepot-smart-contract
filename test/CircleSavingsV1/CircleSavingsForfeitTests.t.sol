...
contract CircleSavingsForfeitTests is CircleSavingsV1Setup {
    ...
    function test_ForfeitMember_Success() public {
        ...
        assertEq(collateralBefore - aliceAfter.collateralLocked, expectedDeduction, "Collateral should be deducted");
    ...
    function _getRecipientForPosition(uint256 _circleId, uint256 _position) internal view returns (address) {
        address[] memory members = circleSavings.getCircleMembers(_circleId);

        for (uint256 i = 0; i < members.length; i++) {
            (CircleSavingsV1.Member memory m,,) = circleSavings.getMemberInfo(_circleId, members[i]);
